# Date and Time: Thursday, February 19, 2026 at 7:09:22 PM WET
...
def executar_cesar():
    ...
    print("\nTarefa 3 (Ataque baseado na frequência de A, E, O):")
    ...
    # Testar as 26 chaves possíveis e contar qual gera mais 'A', 'E' e 'O'
    for k in range(26):
        tentativa = cesar_decifrar(criptograma_t3, k)
        contagem = tentativa.count('A') + tentativa.count('E') + tentativa.count('O')
        
        if contagem > max_ocorrencias:
            max_ocorrencias = contagem
            melhor_chave = k
            melhor_texto = tentativa
            
    print(f"-> Chave descoberta pelo algoritmo: {melhor_chave} (com {max_ocorrencias} ocorrências das letras A, E, O)")
    print(f"-> Texto decifrado: {melhor_texto}")
